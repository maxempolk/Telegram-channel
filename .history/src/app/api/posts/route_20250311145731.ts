...
export async function POST(request: NextRequest) {
  try {
    ...
    const downloadTelegramFile = processedUpdates?.message.images

    if(downloadTelegramFile !== undefined && downloadTelegramFile.length > 0)
      console.log( downloadTelegramFile[0].file_id )
